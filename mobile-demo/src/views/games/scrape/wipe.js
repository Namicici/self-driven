 ...
 *
 */
function ExtendContext2D (prototype) {
    // 绘制矩形
...
export default class Wipe {
    constructor (opts) {
        this.opts = {
            ...
            debug: false,
            data: [],
            onswiping: (percent, data) => {}
        }
        this.doc = document
        for (var i in opts) {
            this.opts[i] = opts[i]
        }
        ExtendContext2D(CanvasRenderingContext2D.prototype)
        this.init()
    }
    $ (name) {
    ...
    wipeStart (ctx, e, self) {
        if (e == undefined) return
        self.startTime = +new Date() // start time
        let x, y
        let xy = self.winTcanvasXY(self.canvas, e.touches[0].pageX, e.touches[0].pageY)
        x = xy.x
        y = xy.y
    ...
    setEvent () {
        ...
        this.canvas.addEventListener('touchstart', (e) => {
            this.wipeStart(ctx, e, this)
        })
        this.canvas.addEventListener('touchmove', (e) => {
            this.wipeMove(ctx, e, this)
        })
        this.canvas.addEventListener('touchend', (e) => {
            this.wipeEnd(ctx, e, this)
        })
    }
    wipePercent (self) {
